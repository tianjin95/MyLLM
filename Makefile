...
$(BUILD_DIR)/chat.o: chat.cpp chat.h cpu_llm.h metal_llm.h model.h | $(BUILD_DIR)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -MMD -MP -c $< -o $@

$(BUILD_DIR)/cpu_llm.o: cpu_llm.cpp cpu_llm.h model.h profiler.h | $(BUILD_DIR)
	$(CXX) $(CPPFLAGS) $(CXXFLAGS) -MMD -MP -c $< -o $@
...
$(BUILD_DIR)/metal_llm.o: metal_llm.mm metal_llm.h metal_llm.metal model.h cpu_llm.h profiler.h | $(BUILD_DIR)
	$(OBJCXX) $(CPPFLAGS) $(CXXFLAGS) $(METAL_CXXFLAGS) -MMD -MP -c $< -o $@

-include $(OBJECTS:.o=.d)

run: $(TARGET)
	./$(TARGET) --model "$(MODEL)" --tokens "$(TOKENS)" $(RAW_FLAG) $(GPU_FLAG) $(SYSTEM_FLAG) $(PROFILE_FLAG)
